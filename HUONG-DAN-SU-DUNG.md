...
|------|-------------------|----------|
| **Bản chạy thẳng (khuyên dùng)** | `NabuPDF-0.2.43-portable.exe` (~372 MB) | Cắm USB, chạy luôn, không cài. Tiện mang đi nhiều máy. |
| **Bản cài đặt** | `NabuPDF-0.2.43-x64.exe` (~372 MB) | Cài cố định vào 1 máy (tạo shortcut, gỡ qua Control Panel). |
...
1. Copy `NabuPDF-0.2.43-portable.exe` vào USB (hoặc ổ cứng máy đích).
...
1. Copy `NabuPDF-0.2.43-x64.exe` vào máy đích.
...
*Phiên bản: 0.2.43 · Đóng gói portable + installer cho Windows x64.*
